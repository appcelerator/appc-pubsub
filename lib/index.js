...
PubSubClient.prototype._retry = function (data, reason, opts) {
	...
	setTimeout(() => this._send(data), Math.max(500, (Math.pow(2, this.retry) - 1) * 500));
...
PubSubClient.prototype._send = function (data) {
	...
	try {
		...
		axios(opts)
			...
			// handle HTTP errors
			.catch(e => {
				// an error which isn't a validation or authorization error, retry sending
				if (e.response.status && ![ 400, 401 ].includes(e.response.status)) {
