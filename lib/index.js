...
class PubSubClient extends EventEmitter {
	...
	constructor(opts) {
		...
		this.#timeout = opts.timeout || DEFAULT.timeout;
		this.#retryLimit = opts.retryLimit || DEFAULT.retryLimit;

		const reconfigureTimeout = opts.reconfigureTimeout || DEFAULT.reconfigureTimeout;
	...
	async _fetchConfig() {
		...
		const response = await fetch(new URL('/api/client/config', this.#url), {
