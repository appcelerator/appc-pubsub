...
function PubSubClient(opts) {
	...
	// batch configs that shouldn't be overwritten by opts
	const batchConfig = {
		maxBytes: Math.pow(1024, 2) // 1m

	}

	if (opts.batching) {
		this.batching = Object.assign({ maxWait: 1000 }, opts.batching, batchConfig);
	}
}
...
PubSubClient.prototype.publish = function (name, data, options) {
	...
	let payloadBytes = Buffer.byteLength(JSON.stringify(payload));

	debug('publish payload size:', payloadBytes, ', current queue:', this.currentBatchBytes);
	if (this.currentBatchBytes + payloadBytes > this.batching.maxBytes) {
		// the first event is bigger than the max byte size, directly send it.
		...
		debug('queue size exceeded, flush currently queued events');
		this._flush();
	}

	// add to the staging queue
	this.queue.push(payload);
	this.currentBatchBytes += payloadBytes;

	// trigger timeout to flush queued events
	if (!this.batchTimeoutHandler) {
		debug('set batch timeout ', this.batching.maxWait);
 ...
 * @private
 */
PubSubClient.prototype._flush = function () {
	debug('flush batch, ', this.queue.length);
...
PubSubClient.prototype._send = function (data) {
	...
	let eventLen = Array.isArray(data.event) ? data.event.length : 1;
	...
	try {
		...
		req.on('response', function (resp) {
			...
			self.counter -= eventLen;
			debug('pending events count: ', self.counter, ', failed events: ', self.failedCounter);
		});
		// handle HTTP errors
