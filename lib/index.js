...
PubSubClient.prototype._requeue = function (reason, opts) {
	if (this.disabled || this._closing) { return; }
...
PubSubClient.prototype.close = function() {
	...
	var self = this;
	if (!this._closed && !this._closing) { this._drainQueue(); }
	if (!this._closing) {
	...
	if (this._shutdownHandler) {
		...
		process.removeEventListener('SIGHUP', this._shutdownHandler);
...
function createDisconnectHandler(self, socket, reconnect) {
	return function disconnectHandler(reason, message) {
		...
		var delay;
		if (!reconnect && !self._pendingExit) {
		...
		if (self._shutdownHandler) {
			...
			process.removeListener('SIGHUP', self._shutdownHandler);
...
PubSubClient.prototype._reconnect = function() {
	...
	process.on('shutdown', this._shutdownHandler);
	process.on('SIGINT', this._shutdownHandler);
	process.on('SIGHUP', this._shutdownHandler);
